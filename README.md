...
**给插件开发者的小工具。** 现在只认本地插件目录——链过去、改完即生效；装 npm 上别人发布的插件还没做。
...
npx dsh-box pull 0.1.0-rc.8                           # 下载一个官方版本
...
下载的版本、所有沙箱、过程日志，全在一个叫 `data` 的文件夹里。**数据跟着程序走**：

- `npx` 或从源码跑，它建在你当前所在的目录下，`dsh-box/data`。
- 装了桌面版、或解压了便携包，它就在 exe 旁边的 `dsh-box/` 里。那个文件夹只装两样：`boot` 是程序，`data` 是你的家当。**覆盖解压就是升级——`boot` 换新，`data` 一个字不动**；卸载器也只删它自己装进来的东西。只有当 exe 待在写不进去的地方（比如 Program Files），数据才退到你的用户目录。

想换地方，用 `--box <目录>` 或环境变量 `DSH_BOX_HOME`。如果那个位置已经有个同名文件夹、里面有东西、又不是本工具建的，工具会另起一个名字，不会写进别人的目录。

下载、删除、启动这些要等的动作，每次都在 `data/logs/` 留一份带时间戳的日志，窗口关了也还在。报问题时把对应那份贴出来最省事。
...
这个变量和 `~/.dsh` 这个默认值，自 2026-06-25 在官方仓里第一次出现至今没改过名、没有别名、也没有兼容层。**只要这两样还在，开沙箱这半边就不受 dsh 更新影响。** 把本地插件送进 profile 那半边依赖的目录约定要新得多，那部分由我们自己跟。
