...
上一版有两套门面：窗口一套逻辑，命令行一套逻辑，同一件事写了两遍。它们悄悄跑偏过——命令行起的沙箱，窗口当它不存在。这一版把窗口降级成命令行的另一种视图：**读走磁盘，写走命令**。窗口每按一个按钮，都是起一个进程去敲你也能敲的那条命令。

由此长出了这一版真正的新东西：**Agent 可以驾驶这个工具，而你看得见它在做什么。**
