...
node bin/cli.js drop 0.1.0-rc.6          # 删掉一个已下载的版本
node bin/cli.js sandboxes --json         # 沙箱清单(JSON)
node bin/cli.js start --version 0.1.0-rc.8 --sandbox <名> --workspace <目录> --json
node bin/cli.js start --version 0.1.0-rc.8 --main   # 非沙箱:用真实 ~/.dsh 启动,插件仅本次生效
...
- `adopt` 只复制不移动,重复的对话按 session id 跳过,可安全重跑。检测到 3080 端口有 dsh 在跑会拒绝(用户重启 dsh 前看不到新对话);确认无碍可加 `--force`。该检测只认 3080——跑在自定义端口的 dsh 探不到。
